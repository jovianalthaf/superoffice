  ...
  return (
    <Link href={`/office/${space.slug}`} className="card">
      <div className="flex flex-col rounded-[20px] border border-[#E0DEF7] bg-white overflow-hidden">
        <div className="thumbnail-container  w-full h-[200px] relative">
          {space.tags.map((tag) => (
            <p
              key={tag}
          ...
          {space.tags.map((tag) => (
            <p
              key={tag}
              className="absolute top-5 left-5 w-fit z-10 rounded-full p-[6px_16px] bg-[#0D903A] font-bold text-sm leading-[21px] text-[#F7F7FD]"
            >
              {tag}
            </p>
          ))}
